...
def calculate_gene_scores(file_degs, G_grn, lfc_thresh=1.0, pval_thresh=0.05,
                          prize_multiplier=1.0, mu=0.05, gene_module=[],
                          score_others="padj", custom_hub_penalty=None, hybrid_score=None,
                         penalize_degs=False):
    """
    ...
    print("=============================CALCULATING GENE SCORES GENERIC==============================")
    df = pd.read_csv(file_degs, sep=",")

    df2 = df.copy()
    df2["score"] = -np.log(df2["AdjPValue"]) * prize_multiplier
    if hybrid_score:
        df2["score"] = -np.log(df2["AdjPValue"]) * df2["Log_FoldChange"] * prize_multiplier
    else:
        df2["score"] = -np.log(df2["AdjPValue"]) * prize_multiplier
    dict_scores_orig = dict(zip(df2["Gene_symbol"], df2["score"]))
    ...
    if hybrid_score:
        df_sub["score"] = -np.log(df_sub["AdjPValue"]) * abs(df_sub["Log_FoldChange"]) * prize_multiplier
    else:
        df_sub["score"] = -np.log(df_sub["AdjPValue"]) * prize_multiplier

    # Scores only for DEGs
    ...
    if gene_module:
        if score_others == "mean":
            print("Calculating the mean scores")
            ...
            print("Calculating the min scores")
            if len(dict_scores2) > 0:
                min_score = min(list(dict_scores2.values()))
                for gene in gene_module:
                    if gene not in dict_scores2:
                        dict_scores2[gene] = min_score * prize_multiplier
        elif score_others == "padj":
            print("Calculating the original scores")
            if len(dict_scores2) > 0:
                for gene in gene_module:
                    if gene not in dict_scores2:
                        dict_scores2[gene] = dict_scores_orig[gene]
    ...
    if penalize_degs:
        dict_scores2 = {k:(v - (mu * hub_penalty[k])) for k, v in dict_scores2.items() if k in hub_penalty}
